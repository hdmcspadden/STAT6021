...
Next, apply the y^2 transformation and the sqrt(x) transformation, create the model, and plot the data.
...
```
The model still has more variance in the residuals than I would expect; however, the mean of residuals issue has been addressed. There are no longer two clusters. The residuals are uncorrelated.

I will check the summary and anova tables for the filtered y^2 sqrt(x) model:
