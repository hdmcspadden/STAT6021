...
Bonderroni allows us a 95% CI for all B's instead of just one. If we have n intervals, we want 95% confidence in all n intervals.
...
Page 68:
"the parameter represents the expected change in the response y per unit change in xj when all the remaining regressor variables xi (i != j) are held constant.


**Interpretation of ANOVA / hypothesis tests**
ANOVA F test is not very useful in MLR expect in some settings. Will explore more in Module 5.



**t test in MLR**
tests is an individual coefficient is 0. Can we drop a particular xj from the model in the presence of the other predictors.
